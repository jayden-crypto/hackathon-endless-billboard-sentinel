...
def seed_registry():
    ...
    with open(path, newline='') as f:
        for row in csv.DictReader(f):
            if db.query(models.RegistryBillboard).filter(models.RegistryBillboard.license_id==row['license_id']).first(): continue
            ...
            db.add(item); n+=1
    db.commit(); return {'seeded': n}
@router.get("/registry/{license_id}")
def check(license_id: str):
    db = SessionLocal(); r = db.query(models.RegistryBillboard).filter(models.RegistryBillboard.license_id==license_id).first()
